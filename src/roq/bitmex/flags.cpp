...
ABSL_FLAG(std::string, rest_uri, "https://testnet.bitmex.com", "REST end-point (URI)");

ABSL_FLAG(uint32_t, rest_ping_freq_secs, 5, "ping frequency (seconds)");

ABSL_FLAG(
    std::string, rest_ping_path, "/", "URI path used for REST connection keep-alive messages");
...
ABSL_FLAG(uint32_t, rest_rate_limit_max_requests, 60, "rate limit: max requests (per interval)");

ABSL_FLAG(uint32_t, rest_expires_timeout_secs, 1, "expires time-out (seconds)");

ABSL_FLAG(bool, rest_allow_order_updates, false, "allow inconsistent order updates? (bool)");

// ws

ABSL_FLAG(std::string, ws_uri, "wss://testnet.bitmex.com/realtime", "WebSocket end-point (URI)");
...
ABSL_FLAG(bool, ws_cancel_on_disconnect, true, "cancel orders on disconnect? (bool)");
