...
TEST(json_error_response, simple_1) {
  ...
  auto res = json::ErrorParser::dispatch(message, [](auto &error) {
    EXPECT_EQ(error.message, "Account has insufficient Available Balance, 5929700 XBt required"_sv);
  });
  EXPECT_TRUE(res);
}

TEST(json_error_response, simple_2) {
  const auto message = R"({"error":{"message":"Invalid leavesQty","name":"HTTPError"}})"_sv;
  auto res = json::ErrorParser::dispatch(
      message, [](auto &error) { EXPECT_EQ(error.message, "Invalid leavesQty"_sv); });
  EXPECT_TRUE(res);
}
